...
export class WorkspaceRunManager {
  ...
  async startRun(config: WorkspaceRunConfig): Promise<Result<WorkspaceRunId>> {
    ...
    try {
      ...
      // Create workspace branch so agent commits land here, not on main/master
      const branch = `${BRANCH_PREFIX}${runId}`
      await this.deps.isolator.createBranch(handle, branch)

      // Detect default branch for PR target
      const defaultBranch = await this.deps.isolator.detectDefaultBranch(handle)
  ...
  private subscribeToBus(
    ...
    const unsubCompleted = system.bus.subscribe(
      ...
      async (message) => {
        ...
        try {
          // Look up goal description for PR title
          const goal = await system.goalRepo.findById(message.goalId)
          const goalDesc = goal?.description ?? `workspace ${runId}`
          const prTitle = `[DevFleet] ${goalDesc.slice(0, 60)}`
          const prBody = [
            `## Goal`,
            goal?.description ?? "N/A",
            "",
            `**Workspace run:** ${runId}`,
            `**Goal ID:** ${message.goalId}`,
          ].join("\n")
          ...
          console.error("[WorkspaceRunManager] post-goal delivery failed:", reason)
          await system.bus.emit({
            id: createMessageId(),
