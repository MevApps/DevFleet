...
export class WorkspaceRunManager {
  ...
  async startRun(config: WorkspaceRunConfig): Promise<Result<WorkspaceRunId>> {
    ...
    try {
      ...
      console.log(`[WorkspaceRunManager] creating branch "${branch}" in ${cloneDir}`)
      execFileSync("git", ["checkout", "-b", branch], { cwd: cloneDir })
      console.log("[WorkspaceRunManager] branch created successfully")
