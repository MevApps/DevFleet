    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[["Total Tokens", data.totalTokensUsed.toLocaleString()], ["Total Cost", `$${data.totalCostUsd.toFixed(4)}`], ["Avg Cost/Goal", `$${avgCost.toFixed(4)}`]].map(([l, v]) => (
          <div key={l} className="rounded-lg border border-border bg-card p-4"><div className="text-sm text-text-secondary">{l}</div><div className="text-2xl font-bold text-text-primary mt-1">{v}</div></div>
        ))}
      </div>
      <div className="rounded-lg border border-border bg-card p-4">
        <h2 className="text-lg font-semibold text-text-primary mb-4">Agent Token Spend</h2>
        <div className="space-y-2">{agents.map(([agent, tokens]) => (
          <div key={agent} className="flex items-center gap-3">
            <span className="w-24 text-sm text-text-secondary truncate">{agent}</span>
            <div className="flex-1 h-4 bg-hover rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full" style={{ width: `${(tokens / max) * 100}%` }} /></div>
            <span className="text-sm text-text-primary w-20 text-right">{tokens.toLocaleString()}</span>
          </div>
        ))}</div>
      </div>
    </div>
