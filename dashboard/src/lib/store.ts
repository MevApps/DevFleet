...
interface DashboardState {
  agents: readonly AgentDTO[]; allTasks: readonly TaskDTO[]; goals: readonly GoalDTO[]; recentEvents: readonly EventDTO[]; metrics: MetricsSummary | null; phases: readonly string[]; tasksByPhase: Record<string, readonly TaskDTO[]>
...
export const useDashboardStore = create<DashboardState>((set) => ({
  agents: [], allTasks: [], goals: [], recentEvents: [], metrics: null, phases: [], tasksByPhase: {},
  alerts: [], unreadAlertCount: 0,
  fetchLiveFloor: async () => { const data = await api.liveFloor(); set({ agents: data.agents, allTasks: data.allTasks, recentEvents: data.recentEvents }) },
